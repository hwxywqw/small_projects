#include <stdio.h>
#include <stdlib.h>
#include <string.h>

static const unsigned int
Sbox[] = {
,},

CK[] = {
0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269, 0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249, 0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229, 0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209, 0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279},

FM[] = {
0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc};

unsigned int SubWord(unsigned int Word){
    unsigned int result = 0;
    for (int i = 0; i < 4; i ++)
        result |= Sbox[(Word >> (8 * i)) & 0xFF] << (8 * i);
    return result;
}
unsigned int SubByte(unsigned int Byte){return Sbox[Byte];}
unsigned int CycsLeft(unsigned int Word, int len, int n){return (Word>>(len-n))+((Word&((1<<(len-n))-1))<<n);}
unsigned int LinearF(unsigned int Word){return Word ^ CycsLeft(Word,32,2) ^CycsLeft(Word,32,10) ^ CycsLeft(Word,32,18) ^ CycsLeft(Word,32,24);}
unsigned int LinearK(unsigned int Word){return Word ^ CycsLeft(Word,32,13) ^CycsLeft(Word,32,23);};
unsigned int PermutationT(unsigned int Word, unsigned int (*Linear)(unsigned int)){return Linear(SubWord(Word));}
unsigned int * XOR_128(unsigned int state[4], unsigned int key[4]){
    for (int i = 0; i < 4; i ++){
        state[i] ^= key[i];
    }
    return state;
}

unsigned int FuncF(unsigned int input[4], unsigned int rkey){
    return input[0] ^ PermutationT(input[1]^input[2]^input[3]^rkey, LinearF);
}

void keygenerate(unsigned int key[4], unsigned int roundkey[]){
    unsigned int temp[36] = {0};
    for (int i = 0; i < 4; i ++){
        temp[i] = key[i] ^ FM[i];
    }
    for (int i = 4; i < 36; i ++){
        temp[i] = temp[i-4] ^ PermutationT(temp[i-1]^temp[i-2]^temp[i-3]^CK[i-4], LinearK);
        roundkey[i-4] = temp[i];
    }
}

void SM4_encode(unsigned int input[4], unsigned int roundkey[32]){
    unsigned int temp[36] = {0};
    for (int i = 0; i < 4; i ++){
        temp[i] = input[i];
    }
    for (int i = 4; i < 36; i ++){
        temp[i] = FuncF(&temp[i-4], roundkey[i-4]);
    }
    input[0] = temp[35];
    input[1] = temp[34];
    input[2] = temp[33];
    input[3] = temp[32];
}

void SM4_decode(unsigned int input[4], unsigned int roundkey[32]){
    unsigned int temp[36] = {0};
    for (int i = 0; i < 4; i ++){
        temp[i] = input[i];
    }
    for (int i = 4; i < 36; i ++){
        temp[i] = FuncF(&temp[i-4], roundkey[35 - i]);
    }
    input[0] = temp[35];
    input[1] = temp[34];
    input[2] = temp[33];
    input[3] = temp[32];
}